...
export function PrivacyPolicyPage({ onClose, language = "ko", onLanguageChange = () => {} }) {
  ...
  return (
    <div className="card adminCard privacyPolicyCard" data-i18n-ignore>
      ...
      <div className="privacyPolicyBody">
        <p className="helperText">시행일자: 2026년 8월 30일</p>
        ...
        <ul>
          ...
          <li>
            구름 띄우기 시: 만난 날짜, 시간, 장소, 성별, 헤어·상의·아우터·하의·신발,
          ...
          <li>푸시 알림 발송을 위한 이용자 식별자, 알림 수신 여부</li>
        </ul>
        <p>
          참고: "구름 띄우기"에 입력하는 상대방의 인상착의 정보는 서비스가 수집하는 것이
          아니라 이용자가 자신의 책임 하에 작성·게시하는 정보입니다. 특정인이 식별되거나
          명예를 훼손할 수 있는 내용을 입력한 경우 관련 법령에 따라 작성자 본인이 책임을 질 수
          있으며, 서비스는 신고된 게시물을 확인해 부적절한 내용을 삭제하거나 이용을 제한할 수
          있습니다.
        </p>
        ...
        <ul>
          ...
          <li>
            재학생 인증 시 제출한 이름, 학번, MY DKU 캡처 이미지는 부정 인증·중복 가입 방지 및
            인증 결과에 대한 이의 제기 대응을 위해 인증 처리(승인·거절) 이후에도 삭제하지 않고
            보관합니다. 단, 회원 탈퇴 시에는 함께 삭제됩니다.
          </li>
        ...
        <h3>5. 개인정보 처리 위탁</h3>
        <ul>
          <li>Supabase: 회원 정보, 서비스 데이터베이스, 인증 이미지 저장, Edge Function 실행</li>
          <li>Google Cloud Vision API: MY DKU 이미지의 학번·학과·재학 상태 자동 인식</li>
          <li>
            OneSignal: 푸시 알림 발송을 위한 이용자 식별자 연동 및 새 메시지 알림에 포함되는
            메시지 미리보기(최대 60자) 전송
          </li>
        </ul>
        ...
        <h3>7. 이용자의 권리</h3>
        <p>
          이용자는 언제든지 마이페이지 내 "회원탈퇴" 기능을 통해 탈퇴할 수 있습니다. 탈퇴 시
          내가 보낸 구름·구름 확인 기록·재학생 인증 정보는 삭제되며, 프로필은 식별할 수 없는
          형태로 처리됩니다. 다만 상대방과 주고받은 채팅 내용, 신고·차단 기록은 상대방 보호 및
          부정 이용 방지를 위해 별도로 보관될 수 있습니다. 그 밖의 열람·정정 요청은 아래
          문의처로 연락해 주세요.
        </p>
