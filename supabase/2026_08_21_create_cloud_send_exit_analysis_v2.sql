-- Track where users leave the cloud-send flow.
-- One row represents one cloud-send attempt. Step times are cumulative.

create table if not exists public.cloud_send_exit_logs (
  ...
  step_6_seconds integer not null default 0,
  previous_count integer not null default 0,
  previous_steps text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint cloud_send_exit_logs_exit_step_check
    check (exit_step is null or exit_step between 1 and 6),
  constraint cloud_send_exit_logs_seconds_check
    check (
      ...
      and step_6_seconds between 0 and 180
    )
);

create index if not exists idx_cloud_send_exit_logs_user_id
  on public.cloud_send_exit_logs (user_id);

create index if not exists idx_cloud_send_exit_logs_started_at
  on public.cloud_send_exit_logs (started_at desc);

alter table public.cloud_send_exit_logs enable row level security;

drop policy if exists cloud_send_exit_logs_insert_own
  on public.cloud_send_exit_logs;
drop policy if exists cloud_send_exit_logs_select_own
  on public.cloud_send_exit_logs;
drop policy if exists cloud_send_exit_logs_update_own
  on public.cloud_send_exit_logs;

create policy cloud_send_exit_logs_insert_own
  on public.cloud_send_exit_logs
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy cloud_send_exit_logs_select_own
  on public.cloud_send_exit_logs
  for select
  to authenticated
  using (auth.uid() = user_id);

create policy cloud_send_exit_logs_update_own
  on public.cloud_send_exit_logs
  for update
  to authenticated
  using (auth.uid() = user_id)
...
drop view if exists public."관리_구름보내기_이탈분석_V2";

create view public."관리_구름보내기_이탈분석_V2" as
select
  l.id as "기록번호",
  l.nickname as "나간 사람 닉네임",
  l.target_gender as "나간사람이 고른 성별",
  ...
  public.format_cloud_send_seconds(l.step_6_seconds) as "6페이지에 머문 시간",
